...
mostrarAyuda(){
	...
	echo "		$0 [-h] [-?] [-f backupConfiguracion] -u usuario_manager -p contraseña_manager "
...
"${DIRECTORIO}"ExpectScripts/loadConfiguration.expect "$FICHERO_CONFIGURCION" "$USUARIO_MANAGER" "$CONTRASENA_MANAGER" \
	"$IP_SWITCH" "$PORT_SWITCH" 
comprobarEjecucion $? "Al procesar ${DIRECTORIO}ExpectScripts/loadConfiguration.expect" 
