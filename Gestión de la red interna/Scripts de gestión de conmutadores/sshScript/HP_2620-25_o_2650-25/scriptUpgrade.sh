...
FIRMWARE="${DIRECTORIO}firware.swi"
IMAGEN_DESTINO="primary"
...
mostrarAyuda(){
	...
	echo "		$0 [-h] [-?] -f ficheroFirmware -u usuario_manager -p contraseña_manager "
	echo "		    -H direccion_switch -P puerto_ssh_switch"
	echo "			[-i imagen_destiono]"
	echo "	"
	echo "  -h -? 					Mostar ayuda"
	echo "	-t ficheroFirmware		Indica el fichero de firware que se usara"
	...
	echo "	-i imagen_destiono 		Indica la imagen destino del switch solo puede ser primary ó secondary"
	echo ""
	echo "	Los valores por defectos son "
	echo "		ficheroFirmware=${FIRMWARE}"
	echo "		imagen_destiono=${IMAGEN_DESTINO}"
	exit 1
}
...
if [ "$IMAGEN_DESTINO" != "primary" ] && [  "$IMAGEN_DESTINO" != "secondary" ]; then
	mostrarAyuda "$0"
fi

"${DIRECTORIO}"ExpectScripts/upgradeFirmware.expect "$FIRMWARE" "$USUARIO_MANAGER" "$CONTRASENA_MANAGER" \
	"$IP_SWITCH" "$PORT_SWITCH" "$IMAGEN_DESTINO"
comprobarEjecucion $? "Al procesar ${DIRECTORIO}ExpectScripts/upgradeFirmware.expect" 
