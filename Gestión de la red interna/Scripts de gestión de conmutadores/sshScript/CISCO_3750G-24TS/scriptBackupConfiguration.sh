...
"${DIRECTORIO}"ExpectScripts/backupConfiguration.expect "$FICHERO_CONFIGURCION" "$USUARIO_MANAGER" "$CONTRASENA_MANAGER" \
	"$IP_SWITCH" "$PORT_SWITCH" 
comprobarEjecucion $? "Al procesar ${DIRECTORIO}ExpectScripts/backupConfiguration.expect" 
