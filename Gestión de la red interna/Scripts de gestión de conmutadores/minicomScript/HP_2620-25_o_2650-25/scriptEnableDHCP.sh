...
sed -e "s/USUARIO_MANAGER/$USUARIO_MANAGER/g" \
        -e "s/PASSWORD_MANAGER/$PASSWORD_MANAGER/g"  \
             "${DIRECTORIO}MinicomScripts/scriptEnableDHCP.template" > "${DIRECTORIO}MinicomScripts/scriptEnableDHCP.tmp"


minicom -d  "$TTY_SERIAL" -b "$BAUD_INICIALES" -S "${DIRECTORIO}MinicomScripts/scriptEnableDHCP.tmp"
comprobarEjecucion $? "Al procesar ${DIRECTORIO}MinicomScripts/scriptEnableDHCP.tmp"
