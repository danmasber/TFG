...
mostrarAyuda(){
	...
	echo " 	$0 [-h] [-?] [-f ficheroConfiguracion] [-t tty_serie] [-b baudIncial] [-u usuario_manager] [-p contraseña_manager]"
	echo ""
	echo "  -h -? 					Mostar ayuda"
	echo "	-f ficheroConfiguracion	Indica el fichero de configuracion que se cargara"
	...
	echo "		ficheroConfiguracion=${FICHERO_CONFIGURCION}"
	exit 1
}
...
minicom -d  "$TTY_SERIAL" -b "$BAUD_INICIALES" -S "${DIRECTORIO}MinicomScripts/scriptLoadConfiguration.tmp"
comprobarEjecucion $? "Al procesar ${DIRECTORIO}MinicomScripts/scriptLoadConfiguration.tmp"

sx "${FICHERO_CONFIGURCION}" -vv < "$TTY_SERIAL" > "$TTY_SERIAL"
comprobarEjecucion $(($?*2)) "Al enviar el fichero ${FICHERO_CONFIGURCION}"
