...
sed -e "s/USUARIO_MANAGER/$USUARIO_MANAGER/g" \
        -e "s/PASSWORD_MANAGER/$PASSWORD_MANAGER/g"  \
             "${DIRECTORIO}MinicomScripts/scriptDisableDHCP.template" > "${DIRECTORIO}MinicomScripts/scriptDisableDHCP.tmp"


minicom -d  "$TTY_SERIAL" -b "$BAUD_INICIALES" -S "${DIRECTORIO}MinicomScripts/scriptDisableDHCP.tmp"
comprobarEjecucion $? "Al procesar ${DIRECTORIO}MinicomScripts/scriptDisableDHCP.tmp"
