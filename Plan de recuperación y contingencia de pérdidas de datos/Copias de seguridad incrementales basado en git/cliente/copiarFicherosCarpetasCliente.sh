...
#
# Este script copia en la carpeta configurada en configuracionRepoCliente.cfg(CARPETA_REPOSITORIO) los ficheros y carpetas indicada 
...
logger -is  "$(basename "$0") INFO: Iniciando copia de los ficheros en la carpeta $CARPETA_REPOSITORIO_LOCAL"
...
[ -f "$ficherosYCarptasRepositorio" ] || comprobarEjecucion 1 "No existe el ficherosYCarptasRepositorio.cfg"
...
do
	...
	if [ "$linea" != "" ]; then
		...
		rsync -acPRAX --delete-after "$lineaRsync" "$CARPETA_REPOSITORIO_LOCAL"  > /dev/null  2>&1
		comprobarEjecucion $? "Copiando el fichero/carpeta $linea en $CARPETA_REPOSITORIO_LOCAL"	
	fi
...
cd "$CARPETA_REPOSITORIO_LOCAL" || comprobarEjecucion 1 "Accediendo a $CARPETA_REPOSITORIO_LOCAL"
...
for elemento in  $( find . | grep -vE "^\./\.|^\.$"   | grep -v -f "${ficherosYCarptasRepositorio}Patron" | sort -rn ); do
	...
	if ! grep "^$elemento" "${ficherosYCarptasRepositorio}TMP" > /dev/null  2>&1 ; then
		...
		rm -Rf "$CARPETA_REPOSITORIO_LOCAL$elemento"
		comprobarEjecucion $? "Borrando el fichero/carpeta $elemento en $CARPETA_REPOSITORIO_LOCAL"	
	fi
done

rm  -f "${ficherosYCarptasRepositorio}TMP"
rm -f "${ficherosYCarptasRepositorio}Patron"
logger -is  "$(basename "$0") INFO: Finalizado copia de los ficheros en la carpeta $CARPETA_REPOSITORIO_LOCAL"
